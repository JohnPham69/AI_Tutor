...
* Trình bày được quy tắc gọi tên alkane theo danh pháp thay thế, áp dụng gọi được tên cho một số alkane mạch không phân nhánh và một số alkane mạch nhánh chứa không quá 5 nguyên tử carbon.
...
3. Alkane $\text{X}$ có công thức phân tử $\text{C}_6\text{H}_{12}$. Xác định công thức cấu tạo và gọi tên alkane $\text{X}$, biết $\text{X}$ chỉ có thể tạo ra một dẫn xuất monochloro duy nhất.
