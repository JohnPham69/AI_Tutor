...
b) Dãy số 1; 2; 4; 8 có u_2 - u_1 != u_3 - u_2, nên không phải là cấp số cộng.
...
**Giải**
a) Ta có: u_1 = 2.1 + 1 = 3;
u_(n+1) - u_n = (2(n+1) + 1) - (2n + 1) = 2n + 2 + 1 - 2n - 1 = 2, với mọi n thuoc N*.
Vậy dãy số (u_n) là cấp số cộng với số hạng đầu u_1 = 3 và công sai d = 2.
b) Ta có: v_1 = -3.1 + 5 = 2;
v_(n+1) - v_n = (-3(n+1) + 5) - (-3n + 5) = -3n - 3 + 5 + 3n - 5 = -3, với mọi n thuoc N*.
...
**Giải**
Gọi d là công sai của cấp số cộng, ta có d = b - a = c - b. Do đó b = (a + c)/2.
...
b) Cấp số cộng (b_n) có b_1 = 2 và b_10 = 20.
...
c) Cho cấp số cộng (v_n) có S_2 = -3 và S_3 = -15. Tính S_50.

**Giải**
a) Ta có thể sắp xếp 100 số nguyên dương đầu tiên thành cấp số cộng có u_1 = 1, u_100 = 100.
Suy ra S_100 = 100(1+100)/2 = 50.101 = 5050.
b) Ta có u_4 + u_6 = (u_1 + 3d) + (u_1 + 5d) = 2u_1 + 8d = 20. Suy ra S_9 = 9(2u_1 + 8d)/2 = 9.20/2 = 90.
c) Ta có: S_2 = (2v_1 + d) = -3, suy ra v_1 + d = -1; (1)
S_3 = 3(2v_1 + 2d)/2 = -15, suy ra v_1 + 2d = -3. (2)
Từ (1) và (2) ta được hệ phương trình: {v_1 + d = -1, v_1 + 2d = -3}.
Giải hệ phương trình trên ta được v_1 = 1 và d = -2.
Do đó S_50 = 50(2.1 + 49.(-2))/2 = 50.(2 - 98)/2 = 25.(-96) = -2400.

a) Tính tổng 50 số tự nhiên chẵn đầu tiên.
b) Cho cấp số cộng (u_n) có u_3 + u_26 = 100. Tính tổng 30 số hạng đầu tiên của cấp số cộng đó.
c) Cho cấp số cộng (v_n) có S_6 = 18 và S_10 = 110. Tính S_20.
...
5.  Tìm số hạng đầu và công sai của cấp số cộng (u_n), biết:
    a) {u_3 - u_5 = 20, u_4 + u_5 = 54};
    b) {u_3 + u_5 = 0, u_2 + u_8 = 80};
    c) {u_2 - u_4 = -3, u_3 = 24}.
6.  Một người muốn mua một thanh gỗ đủ để cắt ra làm các thanh ngang của một cái thang đó (từ bậc dưới cùng) lần lượt là 45 cm, 43 cm, 41 cm, ..., 31 cm.
